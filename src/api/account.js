...
export function sendByPathPayment(seed, destination, record, memo_type,memo){
  ...
  return server.loadAccount(address).then(account=>{
    ...
    var memo = getMemo(memo_type, memo_value);
    var tx = new StellarSdk.TransactionBuilder(account, {memo:memo}).addOperation(opt).build();
    tx.sign(StellarSdk.Keypair.fromSecret(seed));
    return server.submitTransaction(tx);
  })
