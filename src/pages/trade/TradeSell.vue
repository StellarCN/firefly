    ...
    <div class="content">
      <card class="mytrade" padding="10px 10px">
        <div class="card-content" slot="card-content">
          ...
          <v-text-field
              name="amount"
              :label="$t('Trade.Total') + buyasset.code"
              :value="total"
              dark
              error
              disabled
          ...
          <!--按钮-->
          <v-btn class="error btn-sell" :loading="working" @click.stop="doSell">{{$t('Trade.Sell')}}{{asset.code}}</v-btn>

          <!--可用-->
          <div class="available">{{$t('Available')}}:{{balance.balance || 0}}&nbsp;{{asset.code}}</div>
...
export default {
  ...
  methods: {
    ...
    doSell(){
      ...
      console.log('--sell option: $s',option)
